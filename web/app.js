...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // =====================================================================================
  // ========== FULLPAGE SNAP (mindig KÖZÉPRE áll) ======================================
  // =====================================================================================
  ...
  // aktuális index: a viewport közepéhez legközelebbi szekció
  ...
  // ===== Görgő / trackpad – érzékeny, de mindig csak 1-et vált
  let wheelLock = false;
  let lastSnapAt = 0;
  const WHEEL_THRESHOLD = 12; // kisebb pöccintésre is vált
  const SNAP_GUARD_MS = 350;  // védőzár, hogy ne pörögjön át többön

  const onWheel = (e) => {
    const now = performance.now();
    const dy = e.deltaY || 0;

    if (Math.abs(dy) < WHEEL_THRESHOLD || (now - lastSnapAt) < SNAP_GUARD_MS) {
      return; // hagyjuk a természetes mikromozgást
    }

    e.preventDefault(); // most átvesszük
  ...
  // ===== CTA (hero) -> következő (mindig középre)
  ...
  // ===== Navbar linkek -> cél szekció (mindig középre)
  ...
  // ===== Snap igazítás – ha két szekció között maradnánk
  let snapTimeout;
  ...
  // =====================================================================================
  // ========== KÁRTYÁK VÍZSZINTES KÖZÉPRE (csak minimális inline, CSS fájlhoz nem nyúlunk)
  // =====================================================================================
  const centerGrids = () => {
    // a rács cellákban a kártyák középre kerülnek
    ['.glass-grid', '.grid-3'].forEach(sel => {
      $$(sel).forEach(grid => {
        grid.style.justifyItems = 'center';  // elemek vízszintesen középre a saját cellában
      });
    });
    // a kártyák szélességét limitáljuk, hogy arányosak maradjanak – prémium hatás
    $$('.grid-3 .card, .glass-grid .glass').forEach(item => {
      item.style.width = '100%';
      item.style.maxWidth = '340px'; // ha nagyobbat szeretnél, állítsd 360/380-ra
    });
  };
  centerGrids();
});
